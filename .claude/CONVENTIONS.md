...
- Every feature is a `FeaturePlugin` subclass in `src/omnia/plugins/<name>/`, registered
  with `@register("<id>")` from `core/registry.py`, and imported in
  `plugins/__init__.py` so the decorator runs at load time.
...
- `plugins/*` → may import `core/*`. **`core/*` must NOT import `plugins/*`.** No
...
- **Persisted models derive from `core/config/base.PersistedModel`** (`extra="allow"`); only
  never-stored payload models use `StrictModel`. Config lives in the *synced* collection, so a
  strict model turns a key written by a newer device into a `ValidationError` that
  `PluginManager` swallows — the feature then silently never enables. See ADR-010.
- **A dialog that rewrites a whole sub-map MUST read `ConfigRepository.raw_section()` and merge
  onto it — never rebuild the map from this build's registry.** `update_section` merges
  shallowly, so a rebuilt map DELETES every entry this version does not know about (a task, a
  note type, a provider added by a newer release, or one the user hand-wrote). Same rule for the
  values inside an entry: keep what you could not render, and keep what you could read but not
  parse. This cost four review rounds on `note_maintenance`; it will recur for the next
  map-shaped plugin.
