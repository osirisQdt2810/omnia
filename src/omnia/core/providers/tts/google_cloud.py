...
class GoogleCloudTTS(TTSProvider):
    ...
    CURATED_VOICES: ClassVar[list[TTSVoice]] = [
        ...
        # The rest of the en-US Neural2 family. Two of nine was an arbitrary sample, and the
        # gap is not cosmetic: a picker that does not list a voice cannot be used to choose it,
        # so a collection whose existing audio was made elsewhere with (say) Neural2-J could not
        # be matched — new fields came out in a different voice from the ones beside them, on
        # the same card. Google's own gender labels.
        TTSVoice(
            "google_cloud",
            "en-US-Neural2-A",
            "English (US)",
