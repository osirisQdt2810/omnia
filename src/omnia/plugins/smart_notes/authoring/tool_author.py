...
def user_tool_system_prompt() -> str:
    ...
    return (
        ...
        "8b. REENTRANCY. `run` may execute CONCURRENTLY WITH ITSELF, on several threads, for "
        "different notes of the same batch. Derive every path you write from `request.note_id` "
        "or from tempfile.mkstemp/TemporaryDirectory — never a fixed scratch filename, and "
        "never a name derived only from the field. Two invocations racing one temp path put "
        "one note's output in another note's field, with no error anywhere. Keep no mutable "
        "state on the class (a ClassVar cache, a counter): the instance is per-resolve but the "
        "class is shared.\n"
        ...
        "BLANK, then it reads the prompt in that case — return "
        "`not str(params.get('<something>_field', '') or '').strip()`'s inverse accordingly.\n\n"
