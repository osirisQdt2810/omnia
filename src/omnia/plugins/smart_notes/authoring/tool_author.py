...
def user_tool_system_prompt() -> str:
    ...
    return (
        ...
        '9. MEDIA. A field that holds `[sound:name.mp3]` or `<img src="name.jpg">` names a '
        'file in the collection\'s media folder, which is `ctx.media_dir()` (it returns "" '
        "when no collection is open — decline with NotApplicable then). Read it with pathlib. "
        "To OUTPUT a new media file, do NOT write it yourself: return "
        "Produced(GenerationResult(kind, data=<bytes>, ext='mp3')) and Omnia writes it into "
        "the collection and puts the right reference in the field. To convert audio (including "
        "the audio track of a VIDEO), use ctx.audio: decode(bytes) -> WAV bytes, encode(WAV "
        "bytes) -> MP3 bytes. So mp4 -> mp3 is encode(decode(mp4_bytes)). Never shell out to "
        "ffmpeg yourself; ctx.audio is the installed runtime and reports a proper error when "
        "it is missing.\n\n"
