  ...
  function updateToolsSummary(tr) {
    ...
    btn.textContent = "";
    // The class and the tooltip are set for BOTH states, before the early return. The button is
    // reused across renders, so returning early left an emptied row showing the new "none" chip
    // under a tooltip still reading "Tried in order until one produces: AI" — the screen went on
    // claiming AI in the one place the fix was supposed to stop it. It also meant
    // `.sn-tools-none` was only ever toggled OFF, so its stylesheet rule never applied.
    btn.classList.toggle("sn-tools-none", !chain.length);
    btn.title = chain.length
      ? "Tried in order until one produces: " + labels.join(" → ")
      : "No tool — this field generates nothing. Click to add one.";
    if (!chain.length) {
    ...
    // "none", not "default": an empty chain is the ABSENCE of a tool now, and the old
    // name described the opposite state. A row that has never been configured arrives
    // here carrying a real `ai` entry, so it is not this one.
  }
